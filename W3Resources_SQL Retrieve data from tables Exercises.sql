...
select winner from nobel_win where year=1970 and subject not in('Physics','Economics');
...
select winner from nobel_win where subject='Physiology' and year<1971
union
select winner from nobel_win where subject='Peace' and year>=1974;
...
select winner from nobel_win where subject not like 'P%' order by winner asc;
select * from nobel_win where subject not like 'P%' order by year desc, winner;

--24. Write a SQL query to find all the details of 1970 winners by the ordered to subject and winner name; but the list contain the subject Economics and Chemistry at last. (Important)
select * from nobel_win where year=1970
order by
case
